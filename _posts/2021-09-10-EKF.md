...
## 基础背景
扩展卡尔曼滤波[1]作为卡尔曼滤波的扩展，在非线性系统的应用中取得了相当的成功。阅读本文前需要读者先行阅读卡尔曼滤波原理\cite[2]并理解其中的正交投影的数学原理。本文数学推导部分采用的标记与卡尔曼论文的标记类似。
...
$$
\begin{align*}
    \pmb{x}^*(t|t) = \pmb{f}(\pmb{x}^*(t-1|t-1)) + \pmb{\Delta}^*(t)\widetilde{\pmb{y}}(t|t-1)    \tag{14}
\end{align*}
$$
...
\begin{align*}
    ...
    &=\pmb{J}_{f(+)}(t-1)(\widetilde{\pmb{x}}(t-1|t-1))+\pmb{u}(t-1)
\end{align*}
$$
...
先验误差协方差矩阵$\pmb{P}^*_{(-)}(t)$与后验误差协方差矩阵$\pmb{P}^*_{(+)}(t)$分别由下列公式给出
...
\bibitem{b1}  A. H. Jazwinski, Stochastic Processes and Filtering Theory, Academic Press, Inc, vol. 64, 1970.
\bibitem{b2} Kalman R E. A new approach to linear filtering and prediction problems[J]. 1960.
\end{thebibliography}
$$
