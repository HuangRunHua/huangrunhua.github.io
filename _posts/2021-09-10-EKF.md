...
$$
\begin{align*}
    \pmb{x}^*(t|t-1) = &\hat{E}\left[\pmb{f}(\pmb{x}(t-1))\\
    &+\pmb{u}(t-1)|\mathcal{Y}(t-1)\right]    \tag{2}
\end{align*}
$$
...
\begin{align}
    ...
    &\;\;\;\;\times E\left[ \widetilde{\pmb{x}}(t|t-1)\widetilde{\pmb{x}}^T(t|t-1) \right]\times \pmb{J}^T_m(\pmb{x}^*(t|t-1)) \\
